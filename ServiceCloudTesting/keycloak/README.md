...
### Prerequisites
```
1.Copy the realm json configmap from the folder "service_cloud_helm/backbone-services/custom-sidecar-keycloak" to "service_cloud_helm/backbone-services/keycloak/templates"  
#Assuming the pwd is: service_cloud_helm/backbone-services/keycloak , run the below command
cp ../custom-sidecar-keycloak/realm-nic.yaml templates

2. Build and push the custom-sidecar image for keycloak if there are changes to themes or plugins. Folder is located at : "service_cloud_helm/backbone-services/custom-sidecar-keycloak"
```

### Helm package installation

Install the helm chart using below command(Update the storageclass,namespace etc other parameters when necessary)

```
helm install keycloak . --set  replicas=3,image.repository=chub.cloud.gov.in/service-cloud-dev/keycloak,image.tag=17.0.1-custom-1.4,pgchecker.image.repository=chub.cloud.gov.in/library/busybox,pgchecker.image.tag=1.31,postgresql-ha.postgresql.image.registry=chub.cloud.gov.in,postgresql-ha.postgresql.image.repository=library/postgresql-repmgr,postgresql-ha.postgresql.image.tag=14.3.0-debian-10-r11,postgresql-ha.pgpool.image.registry=chub.cloud.gov.in,postgresql-ha.pgpool.image.repository=library/pgpool,postgresql-ha.pgpool.image.tag=4.3.2-debian-10-r5 -n keycloak
 ```
